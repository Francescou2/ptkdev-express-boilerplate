if ("serviceWorker" in navigator) {
	navigator.serviceWorker.register("{{ config.site.fullurl }}/sw.js", {scope: "/", updateViaCache: "all"}).then(function(registration) {
		console.log("{{ translare.sw_reg }}");
	});

	navigator.serviceWorker.ready.then(function(registration) {
		console.log("{{ translare.sw_ready }}");
	});
}
	...
	pwa_add_btn.addEventListener("click", (e) => {
		...
		pwa_deferred_prompt.userChoice.then((choice_result) => {
			if (choice_result.outcome === "accepted") {
				console.log("{{ translare.pwa_installed }}");
			} else {
				console.log("{{ translare.pwa_dismiss }}");
			}
